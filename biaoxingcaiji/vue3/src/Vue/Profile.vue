      ...
      <div class="content-area">
        <div class="profile-detail-page">
          ...
          <div class="profile-detail-header">
            ...
            <h2 class="profile-detail-title">{{ user?.displayName || '表型采集' }}</h2>
            <p class="profile-detail-subtitle">{{ user?.role || '农业研究员' }} | {{ user?.department || '表型采集项目组' }}</p>
          </div>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      // 初始化表单数据
      if (authStore.user) {
        profileForm.displayName = authStore.user.displayName || '表型采集'
